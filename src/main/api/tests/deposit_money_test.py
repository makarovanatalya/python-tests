...
class TestDepositMoney:
    ...
    def test_deposit_money(self,  api_manager: ApiManager, user_request: CreateUserResponse, user_account: CreateAccountResponse, amount: float) -> None:
    ...
    def test_deposit_money_to_another_user_account(self,  api_manager: ApiManager, user_request: CreateUserResponse, user_account: CreateAccountResponse) -> None:
        ...
        assert receiver_account.balance == receiver_user_steps.get_account_by_id(receiver_account.id).balance, "Receiver user balance changed"

    def test_deposit_money_to_non_existent_account(self, api_manager: ApiManager, user_request: CreateUserResponse) -> None:
        api_manager.user_steps.deposit_money_incorrectly(RandomData.get_faker().random_digit(), DEFAULT_TRANSFER_AMOUNT, "Unauthorized access to account")

    @pytest.mark.parametrize("amount", [0, -1])
    def test_deposit_invalid_amount(self, api_manager: ApiManager, user_request: CreateUserRequest, user_account: CreateAccountResponse, amount: float) -> None:
    ...
    def test_deposit_beyond_limit(self, api_manager: ApiManager, user_request: CreateUserRequest, user_account: CreateAccountResponse) -> None:
